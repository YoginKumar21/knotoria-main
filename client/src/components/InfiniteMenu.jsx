...
class InfiniteGridMenu {
  ...
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, scale = 1.0) {
    ...
    this.camera.position[2] = 3.8 * scale;
    this.#init(onInit);
  }
  ...
  #onControlUpdate(deltaTime) {
    ...
    let cameraTargetZ = 3.8 * this.scaleFactor;
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  if (!webglSupported) {
    return (
      <div className="w-full h-full p-8 flex flex-col items-center justify-center bg-gradient-to-br from-[#ee7752]/20 via-[#e73c7e]/20 to-[#23d5ab]/20 rounded-3xl border border-oat overflow-hidden">
        <h3 className="font-display text-2xl font-bold text-cocoa mb-6 text-center">Gallery Showcase</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 w-full max-h-[480px] overflow-y-auto pr-2">
          {items.map((item, idx) => (
            <div 
              key={idx} 
              onClick={() => {
              ...
              className="group cursor-pointer bg-white rounded-2xl overflow-hidden border border-oat shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-1"
            >
              <div className="aspect-square w-full overflow-hidden bg-oat relative">
                <img 
                  src={item.image} 
                  alt={item.title} 
              ...
              <div className="p-4">
                <h4 className="font-display font-semibold text-cocoa text-sm line-clamp-1 group-hover:text-clay transition-colors">{item.title}</h4>
                <p className="text-xs text-cocoa/60 line-clamp-1 mt-1">{item.description}</p>
              </div>
            </div>
          ))}
        </div>
